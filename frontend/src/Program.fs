...
let inline tryUnbox<'T> (v: obj) : 'T option = try Some (unbox<'T> v) with _ -> None

let inline getFirst<'T> (o: obj) (names: string list) : 'T option =
  if isNull o then None else
  names
  |> List.tryPick (fun k ->
      let v : obj = o?(k)
...
module Api =
  ...
  // --- FIXED: use Http.request + Http.send so we get HttpResponse (not tuple)
  ...
  // --- FIXED: use Http.request + DELETE; treat 404 as success/no-op (idempotent)
  ...
  // --- FIXED: use Http.request + DELETE so we get HttpResponse
...
let update (msg: Msg) (model: Model) : Model * Cmd<Msg> =
  ...
  | ToggleTheme ->
      { model with Theme = (match model.Theme with Light -> Dark | Dark -> Light) }, Cmd.none
